...
Opening the page uses the local cache (fast). Use **Refresh from Trakt** only when you need a full re-download of watchlist / watched / lists (slow). Refresh is **read-only** — it never writes episode history to Trakt.
